...
// 1️регистрация всех команд через bot.command / функции setup*
...
// 2️middleware: логирование сообщений и обновление активности участников
bot.on("message", async (ctx, next) => {
  ...
  console.log(`4)Пользователь ${ctx.from.username} в чате ${ctx.chat.title} отправил сообщение: ${ctx.message.text}`);
  return next(); // Обязательно, чтобы команды срабатывали
});
